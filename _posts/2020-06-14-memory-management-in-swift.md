...
![How Reference Counting Works]({{site.url}}/images/20200614-memory-management-in-swift/how_reference_counting_works.png){: width="60%" }
<br>
...
Reference Type 에는 Strong, Weak, Unowned 가 있습니다.
Strong Reference 는 Reference Count 를 증가시켜 객체가 메모리에서 해제되지 않도록 합니다.
...
ObjectB, ObjectC 두 객체의 Reference Count 가 0 이 되지 않아 메모리를 계속 차지하고 있습니다.
...
## Stack vs. Heap
Value Type 과 Reference Type 은 저장되는 공간부터 다릅니다. 메모리를 저장하는 공간으로 Stack 과 Heap 이 있는데
...
![Struct vs. Class Array of Struct]({{site.url}}/images/20200614-memory-management-in-swift/struct_vs_class_struct_array.png){: width="60%" }
<br>
<small>출처: [WWDC 2015 Optimizing Swift Performance](https://developer.apple.com/videos/play/wwdc2015/409/)</small>
<br>
class 객체를 다른 변수에 할당하면 같은 객체를 바라봅니다. 위에서 그림으로 보여드렸듯이 Reference Count 만 증가합니다.
...
![Struct vs. Class Array of Struct]({{site.url}}/images/20200614-memory-management-in-swift/struct_vs_class_class_array.png){: width="60%" }
<br>
<small>출처: [WWDC 2015 Optimizing Swift Performance](https://developer.apple.com/videos/play/wwdc2015/409/)</small>
<br>

프로토콜을 구현한 struct 는 일반적인 struct 와는 조금 다른 형태를 가집니다.
{% gist 97ea3d18ca1aaa7dba22b145788f5088 struct_vs_class_protocol.swift %}
...
<br>
![Struct: Protocol Implementation]({{site.url}}/images/20200614-memory-management-in-swift/protocol_implementation_struct.png){: width="60%" }
<br>
<small>출처: [WWDC 2015 Optimizing Swift Performance](https://developer.apple.com/videos/play/wwdc2015/409/)</small>
<br>
...
Heap 에 할당하여 Reference Type 같지만 일반적인 struct 와 동일하게 다른 변수에 할당하면 데이터를 복사합니다.
...
Protocol Witness Table을 이용하여 instance type 에 맞는 함수를 찾아 호출합니다.
...
Value Type 의 매개 변수는 stack 에 복사되어 함수가 끝난 후에 메모리에서 해제됩니다.
...
그래서 Swift 는 inout modifier 를 제공하여 매개 변수로 넘어온 값을 변경할 수 있습니다.
{% gist 97ea3d18ca1aaa7dba22b145788f5088 parameters_in_function_inout.swift %}

Reference Type 은 inout modifier 없이도 값 변경이 가능합니다.
...
Closure 가 메모리에서 해제되지 않는 한 Closure 에서 가리키는 변수들도 같은 생명주기를 가집니다.
Closure 에서 변수 접근은 예상치 못한 Memory leak 을 발생시킬 수 있습니다.
...
일상에서 많이 접할 수 있는 Reference 문제의 예들을 살펴보려 합니다.
<br>
...
Reference Cycle 을 막기 위해 `addCompletion` 내에서는 `[weak self]` 를 선언해주는 것이 좋습니다.
