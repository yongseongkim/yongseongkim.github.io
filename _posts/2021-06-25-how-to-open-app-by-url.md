...
<br>
`custom-app-scheme://url=www.example.com/features` 와 같이 웹페이지 띄우는 처리를 정의해놓으면 다양한 이벤트를 유연하게 대응할 수 있습니다.
...
예를 들면 링크 https://www.facebook.com/watch 클릭 시 페이스북이 설치되어 있으면 페이스북 앱이 뜨고 아니면 패이스북 watch 페이지를 브라우저에서 띄웁니다.
...
Universal Link 를 활성화 시키기 위해서는 서비스 도메인 웹서버에서 apple-app-site-association 이라는 이름의 JSON 파일을 제공해야 합니다. (페이스북은 https://www.facebook.com/apple-app-site-association 에 정의되어 있습니다.)
JSON 파일에는 어떤 앱을 띄울 것인지, 어떤 경로를 처리할 것인지 등 명시가 되어 있습니다. 이 JSON 파일을 AASA(Apple App Site Association) 이라고 합니다.
사용자는 앱스토어에서 앱을 다운받아 설치하면 OS 에서 앱 정보와 등록된 도메인의 AASA 파일을 파싱하여 Universal Link 를 활성화 시킵니다.
시스템은 업데이트 된 내용을 반영하기 위해 주기적으로 AASA 파일을 다운로드 합니다.
